...
public function __construct($appid, $token, $encodingAESKey=''){
		if (empty($appid)) {
			throw new Exception('微信机器人错误：AppID不能为空');
		}
		if (empty($token)) {
			throw new Exception('微信机器人错误：Token不能为空');
		}
		...
		if($encodingAESKey){
			try {
				...
				if (!$key) {
					throw new Exception('微信机器人错误：EncodingAESKey解码失败');
				}
				...
				throw new Exception('微信机器人错误：加密初始化失败 - ' . $e->getMessage());
			}
		}
	}
