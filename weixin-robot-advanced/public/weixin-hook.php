...
class WEIXIN_Hook{
	...
	public static function filter_avatar_data($args){
		...
		if($email && substr($email, -7) === '.weixin'){
			$parts	= explode('@', $email);
			$openid	= $parts[0];
			$appid	= wpjam_remove_postfix($parts[1], '.weixin');
...
");
		...
		if(weixin_has_feature('weixin_reply')){
			$subs['weixin-replies']	= [
				'menu_title'	=> '自定义回复',
				'function'		=> 'list',
				'list_table'	=> 'WEIXIN_Reply_Setting',
			...
			$subs['weixin-develop']	= [
				...
				'order'			=> 3,
				'function'		=> 'option',
				'option_name'	=> 'weixin-develop',
				'option'		=> ['model'=>'WEIXIN_Develop', 'ajax'=> false],
